...
int seek_forward(int gbdata[], int length, int position, int current_mode)
{
	...
	if(gbdata[position] >= 0x80) return GM_CHINESE; /* self added */
	else return GM_MIXED;
...
void add_shift_char(char binary[], int shifty)
{
	...
	if(debug) { debug_print("SHIFT [%d] ", glyph); }
...
int gm_encode(int gbdata[], int length, char binary[], int reader)
{
	...
	do {
		...
		if(next_mode != current_mode) {
			...
			if(debug) {
				switch(next_mode) {
					case GM_CHINESE: debug_print("CHIN "); break;
					case GM_NUMBER: debug_print("NUMB "); break;
					case GM_LOWER: debug_print("LOWR "); break;
					case GM_UPPER: debug_print("UPPR "); break;
					case GM_MIXED: debug_print("MIXD "); break;
					case GM_BYTE: debug_print("BYTE "); break;
				}
			}
		}
...
void gm_add_ecc(char binary[], int data_posn, int layers, int ecc_level, int word[])
{
	...
	for(i = 0; i < (b1 + b2); i++) {
		...
		/* debug_print("block %d/%d: data %d / ecc %d\n", i + 1, (b1 + b2), data_size, ecc_size);*/
	...
	switch(symbol->input_mode) {
		case DATA_MODE:
			for(i=0,j=0; i < length;) {
				if(source[i] > 0x80) 
				{
					gbdata[j] = (source[i] << 8) | source[i+1];
					i += 2;
					j++;
				}
				else
				{
					gbdata[j] = (int)source[i];
					i++;
					j++;
				}
			}

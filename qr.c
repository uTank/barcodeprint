...
void qr_binary(int datastream[], int version, int target_binlen, char mode[], int jisdata[], int length, int gs1, int est_binlen)
{
...
#else
	char * binary = _alloca(est_binlen + 12);
#endif
	...
	do {
		...
		switch(data_block) {
			case 'K':
				...
				if(debug) { debug_print("Kanji block (length %d)\n\t", short_data_block_length); }
				...
				for(i = 0; i < short_data_block_length; i++) {
					...
					if(debug) { debug_print("0x%4X ", prod); }
				}
			...
			case 'B':
				...
				if(debug) { debug_print("Byte block (length %d)\n\t", short_data_block_length); }
				...
				for(i = 0; i < short_data_block_length; i++) {
					...
					if(debug) { debug_print("0x%2X(%d) ", byte, byte); }
				}
			...
			case 'A':
				...
				if(debug) { debug_print("Alpha block (length %d)\n\t", short_data_block_length); }
			...
			case 'N':
				...
				if(debug) { debug_print("Number block (length %d)\n\t", short_data_block_length); }
				...
				while ( i < short_data_block_length ) {
					...
					if(debug) { debug_print("0x%4X (%d)", prod, prod); }
	...
	if(debug) {
		debug_print("Resulting codewords:\n\t");
		for(i = 0; i < target_binlen; i++) {
			debug_print("0x%2X ", datastream[i]);
		}
		debug_print("\n");
	}
}
...
void add_ecc(int fullstream[], int datastream[], int version, int data_cw, int blocks)
{
	...
	for(i = 0; i < blocks; i++) {
		...
		if(debug) {
			debug_print("Block %d: ", i + 1);
			for(j = 0; j < length_this_block; j++) {
				debug_print("%2X ", data_block[j]);
			}
			...
			debug_print(" // ");
			for(j = 0; j < ecc_block_length; j++) {
				debug_print("%2X ", ecc_block[ecc_block_length - j - 1]);
			}
			debug_print("\n");
		}
...
#else
	unsigned char *mask = _alloca(size * size);
	unsigned char *eval = _alloca(size * size);
#endif
...
#else
	int *utfdata = _alloca((length + 1) * sizeof(int));
	int *jisdata = _alloca((length + 1) * sizeof(int));
	char *mode = _alloca(length + 1);
	int *datastream, *fullstream;
	unsigned char *grid;
			...
			case 'B':
				...
				*byte_used = 1;
				...
				for(i = 0; i < short_data_block_length; i++) {
					...
					if(debug) { debug_print("0x%4X ", byte); }
